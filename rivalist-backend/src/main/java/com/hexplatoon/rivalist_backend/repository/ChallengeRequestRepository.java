...
@Repository
public interface ChallengeRequestRepository extends JpaRepository<Challenge, Long> {
     ...
     */
    @Query("SELECT c FROM Challenge c WHERE c.status = com.hexplatoon.rivalist_backend.entity.Challenge.ChallengeStatus.PENDING AND c.expiresAt < :now")
    List<Challenge> findExpiredChallenges(@Param("now") LocalDateTime now);

    /**
     * Find pending challenge between two users
     */
    @Query("SELECT c FROM Challenge c WHERE c.sender = :sender AND c.recipient = :recipient AND c.status = com.hexplatoon.rivalist_backend.entity.Challenge.ChallengeStatus.PENDING")
     ...
     */
    @Modifying
    @Query("UPDATE Challenge c SET c.status = com.hexplatoon.rivalist_backend.entity.Challenge.ChallengeStatus.EXPIRED WHERE c.status = com.hexplatoon.rivalist_backend.entity.Challenge.ChallengeStatus.PENDING AND c.expiresAt < :now")
